...
dùng CHUNG `skills/`, `schemas/`, `scripts/`, `mcp/`. Mục tiêu: tìm và xếp hạng job phù hợp nhất với
**target** + **CV** của ứng viên, song ngữ Việt–Anh. Lấy dữ liệu job qua **Search Engine + web scraping**
(tool `web_search` của Codex — không cần API key) và tùy chọn **In-Group Facebook Crawler** cục bộ.
