...
# Mặc định bám theo plugin, không phụ thuộc process working directory.
...
def configure_workspace_root(workspace_root: Optional[str] = None) -> Path:
    ...
    root = Path(workspace_root).expanduser() if workspace_root else PLUGIN_ROOT
...
def resolve_workspace_path(value: str) -> Path:
    """Resolve đường dẫn tương đối theo workspace thay vì Path.cwd()."""
...
def main():
    ...
    parser.add_argument("--workspace-root", type=str, default=os.environ.get("JOB_MATCHING_WORKSPACE_ROOT"), help="Thư mục làm việc chứa data/")
    ...
    with sync_playwright() as p:
        ...
        if not isinstance(config_data, dict) or not isinstance(config_data.get("groups"), list):
            raise ValueError(f"Config Facebook không hợp lệ: {config_path}")

        groups = [
            group for group in config_data["groups"]
